<template>
    <div class="card p-4 shadow mb-5" style="text-align: center">
        ...
        <v-select placeholder="Pilih Kategori " label="namkat_vocs" :options="listkategori.data"
                        :reduce="(listkategori) => listkategori.namkat_vocs" v-model="query.nama"></v-select>
        ...
        <div class="row justify-content-end">
            ...
            >
            <div>
                <router-link class="btn btn-success rounded"  v-bind:to="{ name: 'AksiKategoriVoc' }" >Tambah Data <i class="bi bi-file-earmark-plus"></i></router-link>
            </div>
        </div>
                ...
                <table class="table table-striped table-hover" id="scrol">
                    <thead style="background-color: #317af7; color: white">
                        <tr>
                            ...
                            <td class="text-center w-25">STATUS KATEGORI</td>
                            <td class="text-center">Aksi</td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(dt, index) in kategori.data">
                            <td>{{ dt.namkat_vocs }}</td>
...
export default {
    ...
    methods: {
        ...
        searchData() {
            this.url = "/api/auth/kategori/voc?";
   ...
   deleteData(id) {
      ...
      if (confirm("Apakah Anda Ingin Menghapus Data Tersebut ?")) {
        axios.get(`/api/auth/kategori/voc/delete/`+ id ,config).then((response) => {
     ...
     detailData(id) {
      ...
      axios.get("/api/auth/kategori/voc/edit/" + id, config).then((response) => {
