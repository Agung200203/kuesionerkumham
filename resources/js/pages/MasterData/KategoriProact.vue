<template>
    <div class="card p-4 shadow mb-5" style="text-align: center">
        <div
            ...
            ">
            <h3>Kategori Proact</h3>
        </div>
        <form  @submit.prevent="searchData()">
         <v-select placeholder="Pilih Kategori " label="namkat_proact" :options="listkategori.data"
                        :reduce="(listkategori) => listkategori.namkat_proact" v-model="query.nama"></v-select>
        <div class="row mt-2">
        ...
        <div class="row justify-content-end">
            ...
            >
            <div>
                <router-link class="btn btn-success rounded"  v-bind:to="{ name: 'AksiKategoriProact' }" >Tambah Data <i class="bi bi-file-earmark-plus"></i></router-link>
            </div>
        </div>
...
export default {
    ...
    methods: { 
        ...
        const config={
         ...
         searchData() {
            this.url = "/api/auth/kategori/proact?";
   ...
   deleteData(id) {
      ...
      if (confirm("Apakah Anda Ingin Menghapus Data Tersebut ?")) {
        axios.get(`/api/auth/kategori/proact/delete/`+ id ,config).then((response) => {
    ...
    detailData(id) {
      ...
      axios.get("/api/auth/kategori/proact/edit/" + id, config).then((response) => {
