    ...
    <div class="card-body">
      <form @submit.prevent="loadingData()">
        <div class="mb-3">
          ...
          <v-select label="namkat_speed" :options="arrayKategori" :reduce="(arrayKategori) => arrayKategori.id"
            v-model="kuesioner.id_katspeed"></v-select>
        </div>
        <div class="mb-3">
          <p class="form-label">Kuesioner *</p>
          <input class="form-control" v-model="kuesioner.namkuesspeed" required />
        </div>
        <div class="mb-3">
...
export default {
  ...
  methods: {
    getAutocomplete() {
      ...
      axios.get("/api/auth/kues/speed/list/kategori", config)
      .then((response) => {
    ...
    getData() {
      ...
      axios.get("/api/auth/kues/speed/edit/" + this.id, config)
        .then((response) => {
          this.kuesioner = {
            id_katspeed: response.data.data.id_katspeed,
            namkuesspeed: response.data.data.namkuesspeed,
    ...
    storeData() {
      ...
      axios.post("/api/auth/kues/speed/store", this.kuesioner, config).then((response) => {
    ...
    updateData() {
      ...
      if (typeof (this.kuesioner.id_katspeed) === 'string') {
        var idKat = this.kuesioner.id_katspeed
        var result = this.arrayKategori.find(function (e) { return e.namkat_speed == idKat });
        this.kuesioner.id_katspeed = result.id;
      };
      axios.post("/api/auth/kues/speed/update/" + this.id, this.kuesioner, config)
        .then((response) => {
