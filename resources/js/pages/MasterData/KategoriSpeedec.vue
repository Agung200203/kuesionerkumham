<template>
    <div class="card p-4 shadow mb-5" style="text-align: center">
        <div
            ...
            "
        >
            <h3>Kategori Speedec</h3>
        </div>
        <form  @submit.prevent="searchData()">
         <v-select placeholder="Pilih Kategori " label="namkat_speed" :options="listkategori.data"
                        :reduce="(listkategori) => listkategori.namkat_speed" v-model="query.namaSpeed"></v-select>
        <div class="row mt-2">
        ...
        <div class="row justify-content-end">
            ...
            >
            <div>
                <router-link class="btn btn-success rounded"  v-bind:to="{ name: 'AksiKategoriSpeedec' }" >Tambah Data <i class="bi bi-file-earmark-plus"></i></router-link>
            </div>
        </div>
...
export default {
    ...
    methods: { 
        ...
        const config={
         ...
         searchData() {
            this.url = "/api/auth/kategori/speed?";
       if (this.query.namaSpeed !== null) {
                this.url = this.url + "&query=" + this.query.namaSpeed;
            }
   ...
   deleteData(id) {
      ...
      if (confirm("Apakah Anda Ingin Menghapus Data Tersebut ?")) {
        axios.get(`/api/auth/kategori/speed/delete/`+ id ,config).then((response) => {
    ...
    detailData(id) {
      ...
      axios.get("/api/auth/kategori/speed/edit/" + id, config).then((response) => {
