<template>
    <div class="card p-4 shadow mb-5" style="text-align: center">
        ...
        <form @submit.prevent="searchData()">
            <div class="row ">
                <div class="col">
                    <v-select label="namkuesvocs" :options="arrayKuesioner" v-model="query.query"
                        placeholder="Pilih Kuesioner" :reduce="(arrayKuesioner) => arrayKuesioner.namkuesvocs">
                    </v-select>
                </div>

                <div class="col">
                    <v-select label="namkat_vocs" :options="arrayKategori" v-model="query.id_katvocs"
        ...
        <div class="row justify-content-end">
            <div class="mb-1 d-flex justify-content-end" style="width: 310px">
                <div>
                    <router-link class="btn btn-success rounded" v-bind:to="{ name: 'AksiKuesionerVoc' }">Tambah Data <i
                            ...
                            <div class="modal-body">
                                <div class="row pt-3">
                                    ...
                                    <div class="col-md text-start">
                                        : {{ detailKuesioner.namkat_vocs }}
                                    </div>
                                </div>
                                <div class="row pt-3">
                                    <div class="col-md-3 text-start"><b>Kuesioner</b></div>
                                    <div class="col-md text-start">
                                        : {{ detailKuesioner.namkuesvocs }}
                                    </div>
                                </div>
                                <div class="row pt-3">
...
export default {
    ...
    methods: {
        getData() {
            ...
            axios.get("/api/auth/kues/voc/list/kategori", config).then((response) => {
                this.arrayKategori = response.data;
            });
            axios.get("/api/auth/kues/voc/list/kuesioner", config).then((response) => {
        ...
        deleteData(id) {
            ...
            if (confirm("Apakah Anda Ingin Menghapus Data Tersebut ?")) {
                axios.get(`/api/auth/kues/voc/delete/` + id, config).then((response) => {
        ...
        searchData() {
            ...
            if (this.query.id_katvocs !== null) {
                this.url = this.url + "&id_katvocs=" + this.query.id_katvocs;
            }
        ...
        detailData(id) {
            ...
            axios.get("/api/auth/kues/voc/edit/" + id, config).then((response) => {
                this.detailKuesioner = response.data.data;
                this.detailKuesioner = Object.assign(this.detailKuesioner, { namkat_vocs: this.detailKuesioner.kategori.namkat_vocs });
